<div align="center">

# 👋 Hi, I'm Darshan Patil

### Cyber Security Student | Python Developer | Network Security Enthusiast

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=1000&center=true&vCenter=true&width=700&lines=Cyber+Security+%7C+Python+%7C+Networking;Building+Practical+Security+Projects;Learning+%7C+Building+%7C+Securing" />

<br>

<img src="https://komarev.com/ghpvc/?username=darshanp143&label=Profile%20Views&color=0e75b6&style=flat-square" alt="Profile Views" />

</div>

---

## 🧑‍💻 About Me

🎓 Computer Science (Cyber Security) student focused on building practical projects in cybersecurity, networking, and software development.

- 🔐 Interested in Cyber Security, Network Security & Ethical Hacking
- 🐍 Building security tools and automation with Python
- 📡 Exploring networking, packet analysis and traffic analysis
- 🌐 Developing web applications and interactive dashboards
- 🧪 Working with PCAP, Scapy and network-analysis workflows
- ☁️ Exploring Cloud & Infrastructure Security
- 🚀 Learning by building real-world projects

> **Build. Learn. Secure. Repeat.**

---

## 🛠️ Tech Stack

### 💻 Programming

<p>
<img src="https://skillicons.dev/icons?i=python,c,cpp,java,javascript" />
</p>

### 🔐 Cyber Security & Networking

<p>
<img src="https://skillicons.dev/icons?i=linux,kali,bash" />
</p>

`Scapy` `Wireshark` `Nmap` `PCAP Analysis` `IPv4` `IPv6` `TCP/IP`

### 🌐 Web Development

<p>
<img src="https://skillicons.dev/icons?i=html,css,javascript,streamlit" />
</p>

### ☁️ Tools & Platforms

<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,gcp,docker" />
</p>

---

## 🚀 Featured Project

### 🔐 IPv6 OS Fingerprinting Lab

A cybersecurity lab project for passive operating-system fingerprinting using IPv6 network traffic and PCAP files.

**Highlights**

- 📡 IPv6 / ICMPv6 packet analysis
- 🔎 OS fingerprint classification
- 📂 PCAP file processing
- 🐍 Python + Scapy implementation
- 📊 Interactive Streamlit dashboard

**Technologies**

`Python` `Scapy` `IPv6` `ICMPv6` `PCAP` `Streamlit`

---

## 🔥 Contribution Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=darshanp143&theme=tokyonight&hide_border=true" />

</div>

---


## 🐍 Contribution Activity

<div align="center">

<img src="https://raw.githubusercontent.com/darshanp143/darshanp143/output/github-snake-dark.svg"
alt="GitHub Contribution Snake">

</div>

---

## 🎯 Current Focus

- 🔐 Cyber Security & Network Security
- 🐍 Python Security Automation
- 📡 Network Traffic Analysis
- 🌐 Web Application Security
- ☁️ Cloud Security
- 🧠 Machine Learning for Cyber Security
- 🧩 Data Structures & Algorithms

---

## 📚 Currently Learning

`Cyber Security` · `Networking` · `Python` · `Linux` · `Cloud Security` · `DSA`

---

## 📫 Connect With Me

<div align="center">

<a href="https://github.com/darshanp143.">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
</a>

<a href="https://www.linkedin.com/in/darshanp143">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
</a>

</div>

---

<div align="center">

### 🔐 Build. Learn. Secure. Repeat.

⭐ Feel free to explore my repositories and projects.

</div>
